/**
 * @lacspace/analytics-lite — consent & Do-Not-Track helpers.
 *
 ...
 * on the server (where you pass consent explicitly). Zero dependencies.
 */
...
export interface ConsentContext {
  ...
  respectDNT?: boolean;
  /** When consent is unknown (`undefined`), may we track? Default `true`. */
...
export function detectDNT(): boolean | undefined {
  ...
  const raw = (nav && (nav["doNotTrack"] ?? nav["msDoNotTrack"])) ?? g.doNotTrack;
 ...
 * - When `consent` is unknown, `defaultConsent` decides (default `true`, matching
 *   this package's cookieless, no-banner posture).
 */
export function shouldTrack(ctx: ConsentContext = {}): boolean {
  ...
  return ctx.defaultConsent ?? true;
}
