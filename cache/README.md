...
- 📦 `createCache({ max, ttl })` — LRU + per-entry TTL, `get`/`set`/`has`/`delete`/`clear`/`keys`/`size`
- 🔁 `cache.wrap(key, fn)` — cache an async call and **de-dupe concurrent callers** into one fetch
