 ...
 */
import { ConvertError, safeSet, isForbiddenKey } from "./util";
...
function parseFlow(s: string): JsonValue {
  ...
  const parseArr = (): JsonValue => {
    ...
    while (i < s.length) {
      ...
      if (s[i] === ",") { i++; continue; }
      if (s[i] === "]") { i++; break; }
      throw new ConvertError(`YAML: malformed flow sequence near "${s.slice(i)}"`);
    }
    return arr;
  };
  const parseObj = (): JsonValue => {
    ...
    while (i < s.length) {
      ...
      skipWs();
      if (s[i] !== ":") throw new ConvertError(`YAML: expected ':' in flow map near "${s.slice(i)}"`);
      i++;
      ...
      if (s[i] === ",") { i++; continue; }
      if (s[i] === "}") { i++; break; }
      throw new ConvertError(`YAML: malformed flow map near "${s.slice(i)}"`);
    }
    return obj;
  };
...
function splitKey(content: string, n: number): { key: string; rest: string } {
  ...
  throw new ConvertError(`YAML: expected "key: value" mapping at line ${n}: "${content}"`);
}
